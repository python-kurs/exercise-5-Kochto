...
dataset = xr.open_dataset(input_dir / "0.25deg_reg_v19.0e.nc")
seldata = dataset.sel(latitude = slice(30,72),longitude= slice(-13, 25),time = slice ("1981-01-01", "2010-12-31"))
means = seldata.groupby("time.month").mean("time")
...
sel2018 = dataset.sel(latitude = slice(30,72),longitude= slice(-13, 25),time = slice ("2018-01-01", "2018-12-31")).groupby("time.month").mean("time")
anomalies2018 = sel2018 - means
...
meanAnamalie = anomalies2018.mean()["tg"]
coords = [8.77, 50.8]
...
anomalies2018.to_netcdf(output_dir / "europe_anomalie_2018.nc")
